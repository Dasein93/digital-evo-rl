...
- `train/tools/coevolve.py` — N-generation co-evolutionary loop. Each generation alternates predator and prey phases: spawn ``n_mutants`` weight-perturbed copies of the current champion, greedy-eval each against the *current opponent champion* (and optionally a sampled set of *historical* opponents — see Hall of Fame below), fill a per-(gen, team) MAP-Elites archive, promote highest-fitness elite. Writes ``gen_NNN/champion_checkpoint/`` per generation and a cross-generation champion tournament + fitness/coverage plots at the end. Supports ``--workers W`` (ProcessPoolExecutor over mutants; each worker pins ``torch.set_num_threads(1)`` to avoid contention) and ``--resume`` (scans ``out_dir`` for the highest finished ``gen_NNN/champion_checkpoint/manifest.json`` and continues from there; persists an in-progress ``coevolve_summary.json`` after every generation so a crash loses at most one generation). **Hall of Fame** (``--hof_k K``): an in-memory ring of all past champion snapshots; each mutant is also eval'd against ``K`` randomly-sampled historical opponents and selection uses the mean fitness. Fixes the non-transitive cycling visible without HoF (predator champion fitness oscillating instead of monotonically climbing). BC for archive insertion is computed from the current-opponent matchup only, so cell semantics stay stable across runs. The HoF is rehydrated from on-disk champion ckpts on ``--resume``.
...
### CLI surface
- `python run_cpu.py --config <cfg> [--episodes N] [--save_dir D] [--device cpu|cuda|mps|auto] [--seed N] [--num_threads K]`
...
- `python -m train.tools.coevolve --seed_ckpt <ckpt> --out <dir> --generations G --n_mutants N --sigma S [--n_obstacles K] [--workers W] [--resume] [--num_threads K] [--hof_k K] [--hof_eval_eps E]`
- `python -m train.tools.report --dir <run> --out report.html [--title T]`

### VPS deployment
`./scripts/vps_bootstrap.sh` (run from a Mac with SSH access to the box) rsyncs the repo to `/opt/digital-evo-rl/`, installs CPU-only PyTorch and SDL2 system libs, and runs `pytest -q` on the host. `./scripts/run_worker.sh` is the tmux entrypoint for unattended multi-day runs — `nice -n 19`, unbuffered Python, `coevolve --resume`, 60s sleep + restart on crash. Knobs are env vars (see `docs/vps.md`).
